...
const OptionsMenu: React.FC<OptionsMenuProps> = ({
  ...
  return (
    <div className={`absolute z-20 ${isFullScreen 
      ...
      >
        {isFullScreen ? (
          <svg className="w-[20px] fill-gray-800 dark:fill-gray-200" viewBox="0 0 24 24">
          ...
          <svg className="w-[20px] pointer-events-none fill-gray-800 dark:fill-gray-200" viewBox="0 0 1024 1024">
      ...
      {menuOpen && !isFullScreen && (
        <div
          ref={menuRef}
          className={`absolute ${left ? "left-0" : "right-0"} w-48 bg-white dark:bg-[#0C1A28] dark:border-gray-600 rounded-md shadow-lg z-10 border-2`}
        >
          <button
            onClick={() => {
              onDownload();
