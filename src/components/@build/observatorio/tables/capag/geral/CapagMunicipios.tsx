...
const CapagMunicipios = ({ data = [], color = '#000000' }: any) => {
  // -1, 0, 1
  // asc, null, desc
  ...
  const [ordenation, setOrdenation] = useState([
    { index: 1, name: 'valor', ordenation: 0 },
  ])
  ...
  const getRows = (values: any) => {
    ...
    values.map((obj: any) => {
      rows.push([obj['indicador'], obj['valor'], obj['nota']] as string[])
    })
    return rows
  }
